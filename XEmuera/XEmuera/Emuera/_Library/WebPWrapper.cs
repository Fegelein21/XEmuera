...
/// 
/// Encode Functions: (If you need there, define 'ENCODE' keyword. )
...
///
/// Exception classes:
/// WebPException : base Exception class, or any Exceptions.
/// WebPLibException : Any exceptions caused from library or functions.
/// WebPDllNotFoundException : Webp dll cound not load Exception. DllNotFoundException Or EntryPointNotFoundException or so.
/// WebPDecoderException : Any Exceptions in Decoder or doing decoding operations.
/// WebPEncoderException : Any Exceptions in Encoder or doing encoding operations.
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
///
/// Modification:
/// Fix:    Fix: WebP.Decode: change return value "output_buffer" to IntPtr. (2021/02/18 By 東etoマン)
/// Fix:    Fix: Functions: change input/return parameters defined "size_t" match-up to UIntPtr or ulong. (2021/02/21 by M)
/// Add:    Add: WebP*Exception classes for Error handling on AppContents.cs. (2021/03/01 by M)
/// Add:    Add: Webp.Load/Save: IO.Exception retry operation.  (2021/02/26 by M)
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
using System;
...
namespace WebPWrapper
{
    ...
    public sealed class WebP : IDisposable
    {
        ...
        {
#if DEBUG
            System.Diagnostics.Debug.Assert((long)WEBP_MAX_DIMENSION * (long)WEBP_MAX_DIMENSION * 4L < (long)int.MaxValue,
                    $"WARNING!! WEBP_MAX_DIMENSION is expanded to {(long)WEBP_MAX_DIMENSION}, so WebpWrapper may caused to be memory-overflow."
                );
#endif
        }
        ...
        public Bitmap Load(string pathFileName)
        {
            ...
            for (var i = 0; i < MAX_TRY; i++ ) {
                ...
                catch (IOException ex) when (ex is PathTooLongException || ex is DirectoryNotFoundException || ex is FileNotFoundException)
                {
                    // これらのIO例外は即NG
                    throw ex;
                }
                catch (IOException ex)
                {
                    ...
                    if (i < MAX_TRY)
                    {
                        System.Diagnostics.Debug.WriteLine("WebP file Access Error, Retry Count " + i.ToString());
                        Task.Delay(50);
                    }
                    else
                        throw ex;
                ...
                catch (Exception ex) { throw new WebPException(ex.Message + "In Load", ex); }
            }
            System.Diagnostics.Debug.Assert(rawWebP != null, "NULL BUG in WebP.Load");
            try
            {
                return Decode(rawWebP);
            }
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.Load"); }
            catch (EntryPointNotFoundException) { throw new WebPDllNotFoundException("in WebP.Load"); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPDecoderException(ex?.Message + " in WebP.Load", ex); }
        }
        ...
        public Bitmap Decode(byte[] rawWebP)
        {
            ...
            try
            {
                ...
                // WARNING: expanding 48bpp/64bpp color may be caused memmory crash!
                if (hasAlpha)
                ...
                long outputSize = bmpData.Stride * imgHeight;
                IntPtr ptrData = pinnedWebP.AddrOfPinnedObject();
                if (bmp.PixelFormat == PixelFormat.Format24bppRgb)
                    output_buffer = UnsafeNativeMethods.WebPDecodeBGRInto(ptrData, rawWebP.Length, bmpData.Scan0, outputSize, bmpData.Stride);
                else
                    output_buffer = UnsafeNativeMethods.WebPDecodeBGRAInto(ptrData, rawWebP.Length, bmpData.Scan0, outputSize, bmpData.Stride);
                if (output_buffer == IntPtr.Zero)
                    throw new WebPLibException();
        ...
        public Bitmap Decode(byte[] rawWebP, WebPDecoderOptions options)
        {
            ...
            try
            {
                ...
                {
                    throw new WebPLibException("WebPInitDecoderConfig failed. Wrong version?");
                }
                ...
                if (options.use_scaling == 0)
                {
                    ...
                    if (result != VP8StatusCode.VP8_STATUS_OK)
                        throw new WebPLibException(message: $"Failed WebPGetFeatures with error {result}");
                    ...
                    if (options.use_cropping == 1)
                    {
                        if (options.crop_left + options.crop_width > config.input.Width || options.crop_top + options.crop_height > config.input.Height)
                            throw new WebPLibException("Crop options exceded WebP image dimensions");
                ...
                // Decode
                {
                    ...
                    if (result != VP8StatusCode.VP8_STATUS_OK)
                    {
                        throw new WebPDecoderException("Failed WebPDecode with error " + result.ToString());
                    }
                    UnsafeNativeMethods.WebPFreeDecBuffer(ref config.output);
                }
                return bmp;
            }
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.Decode"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.Decode", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPDecoderException(ex?.Message, ex); }
            finally
            {
                //Unlock the pixels
                if (bmpData != null)
        ...
        public Bitmap GetThumbnailFast(byte[] rawWebP, int width, int height)
        {
            ...
            try
            {
                ...
                VP8StatusCode result = UnsafeNativeMethods.WebPDecode(ptrRawWebP, rawWebP.Length, ref config);
                if (result != VP8StatusCode.VP8_STATUS_OK)
                    throw new WebPDecoderException("Failed WebPDecode with error " + result);

                UnsafeNativeMethods.WebPFreeDecBuffer(ref config.output);

                return bmp;
            }
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.GetThumbnailFast"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.GetThumbnailFast", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPDecoderException("GetThumbnailFast", ex); }
            finally
            {
                //Unlock the pixels
                if (bmpData != null)
        ...
        public Bitmap GetThumbnailQuality(byte[] rawWebP, int width, int height)
        {
            ...
            try
            {
                ...
                VP8StatusCode result = UnsafeNativeMethods.WebPGetFeatures(ptrRawWebP, rawWebP.Length, ref config.input);
                if (result != VP8StatusCode.VP8_STATUS_OK)
                    throw new WebPLibException("Failed WebPGetFeatures with error " + result);
            ...
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in Thumbnail"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in Thumbnail", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPDecoderException("Thumbnail", ex); }
            finally
            {
                //Unlock the pixels
                if (bmpData != null)
        ...
        public void Save(Bitmap bmp, string pathFileName, int quality = 75)
        {
            ...
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.Save"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.Save", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPEncoderException(ex?.Message + " in WebP.Save",ex); }
            try
            {
                for (var i = 0; i < 3; i++)
                {
                    ...
                    catch (IOException ex ) when (ex is PathTooLongException || ex is DirectoryNotFoundException)
                    { throw; }
                    catch (IOException)
                    { 
                        if (i < 3) 
            ...
            catch (Exception ex) { throw new IOException(ex.Message + " in WebP.Save", ex); }
        }
        ...
        public byte[] EncodeLossy(Bitmap bmp, int quality = 75)
        {
            ...
            try
            {
                ...
                if (size == 0)
                    throw new WebPEncoderException("Can´t encode WebP");
            ...
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.EncodeLossly"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.EncodeLossly", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPEncoderException(ex?.Message + " in WebP.EncodeLossly", ex); }
            finally
            {
                //Unlock the pixels
                if (bmpData != null)
        ...
        public byte[] EncodeLossy(Bitmap bmp, int quality, int speed, bool info = false)
        {
            ...
            if (UnsafeNativeMethods.WebPConfigInit(ref config, WebPPreset.WEBP_PRESET_DEFAULT, 75) == 0)
                throw new WebPLibException("Can´t config preset");
        ...
        public byte[] EncodeLossless(Bitmap bmp)
        {
            ...
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.EncodeLossless (Simple)"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.EncodeLossless (Simple)", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPException(ex?.Message + " in WebP.EncodeLossless (Simple)", ex); }
            finally
            {
                //Unlock the pixels
                if (bmpData != null)
            ...
            try
            {
                ...
                if (UnsafeNativeMethods.WebPGetDecoderVersion() > 1082)
                {
                    if (UnsafeNativeMethods.WebPConfigLosslessPreset(ref config, speed) == 0)
                        throw new WebPLibException("Can´t config lossless preset");
                }
                else
                {
                    config.lossless = 1;
        ...
        public byte[] EncodeNearLossless(Bitmap bmp, int quality, int speed = 9)
        {
            //test dll version
            if (UnsafeNativeMethods.WebPGetDecoderVersion() <= 1082)
                throw new WebPLibException("This dll version not suport EncodeNearLossless");
        ...
        public string GetVersion()
        {
            ...
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.GetVersion"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.GetVersion", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPLibException("In WebP.GetVersion", ex); }
        }
        ...
        public void GetInfo(byte[] rawWebP, out int width, out int height, out bool has_alpha, out bool has_animation, out string format)
        {
            ...
            try
            {
                ...
                if (result != 0)
                    throw new WebPLibException(result.ToString());
            ...
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.GetInfo"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.GetInfo", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPException(ex?.Message + " in WebP.GetInfo", ex); }
            finally
            {
                //Free memory
                if (pinnedWebP.IsAllocated)
        ...
        public float[] GetPictureDistortion(Bitmap source, Bitmap reference, int metric_type)
        {
            ...
            {
                if (source == null)
                    throw new ArgumentException("Source picture is void");
                if (reference == null)
                    throw new ArgumentException("Reference picture is void");
                if (metric_type > 2)
                    throw new ArgumentException("Bad metric_type. Use 0 = PSNR, 1 = SSIM, 2 = LSIM");
                if (source.Width != reference.Width || source.Height != reference.Height)
                    throw new ArgumentException("Source and Reference pictures have diferent dimensions");
                ...
                if (UnsafeNativeMethods.WebPPictureInitInternal(ref wpicSource) != 1)
                    throw new WebPLibException("Can´t init WebPPictureInit");
                ...
                if (sourceBmpData.PixelFormat == PixelFormat.Format32bppArgb)
                {
                    ...
                    if (UnsafeNativeMethods.WebPPictureImportBGRA(ref wpicSource, sourceBmpData.Scan0, sourceBmpData.Stride) != 1)
                        throw new WebPLibException("Can´t allocate memory in WebPPictureImportBGRA");
                }
                else
                {
                    ...
                    if (UnsafeNativeMethods.WebPPictureImportBGR(ref wpicSource, sourceBmpData.Scan0, sourceBmpData.Stride) != 1)
                        throw new WebPLibException("Can´t allocate memory in WebPPictureImportBGR");
                }
                ...
                if (UnsafeNativeMethods.WebPPictureDistortion(ref wpicSource, ref wpicReference, metric_type, ptrResult) != 1)
                    throw new WebPLibException("Can´t measure.");
                return result;
            }
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.GetPictureDistortion"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.GetPictureDistortion", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new WebPLibException( ex.Message + " at WebP.GetPictureDistortion", ex); }
            finally
            {
                //Unlock the pixels
        ...
        private byte[] AdvancedEncode(Bitmap bmp, WebPConfig config, bool info)
        {
            ...
            try
            {
                ...
                if (UnsafeNativeMethods.WebPValidateConfig(ref config) != 1)
                    throw new WebPLibException("Bad config parameters");
                ...
                dataWebp = new byte[dataWebpSize];                //Memory for WebP output
                ...
                if (UnsafeNativeMethods.WebPEncode(ref config, ref wpic) != 1)
                    throw new WebPEncoderException(((WebPEncodingError)wpic.error_code).ToString());
            ...
            catch (DllNotFoundException) { throw new WebPDllNotFoundException("in WebP.AdvancedEncode"); }
            catch (EntryPointNotFoundException ex) { throw new WebPDllNotFoundException("in WebP.AdvancedEncode", ex); }
            catch (WebPException) { throw; }
            catch (Exception ex) { throw new Exception(ex.Message + " in WebP.AdvancedEncode"); }
            finally
            {
    ...
    internal sealed partial class UnsafeNativeMethods
    {
        ...
        [Obfuscation(Exclude = false)]
        public static IntPtr WebPDecodeBGRInto(IntPtr data, long data_size, IntPtr output_buffer, long output_buffer_size, int output_stride)
        {
            switch (IntPtr.Size)
            {
                case 4:
                    return WebPDecodeBGRInto_x86(data, (UIntPtr)data_size, output_buffer, (UIntPtr)output_buffer_size, output_stride);
                case 8:
                    return WebPDecodeBGRInto_x64(data, (UIntPtr)data_size, output_buffer, (UIntPtr)output_buffer_size, output_stride);
                default:
                    throw new InvalidOperationException("Invalid platform. Can not find proper function");
            }
        }
        ...
        /// <returns>output_buffer if function succeeds; NULL otherwise</returns>
        [Obfuscation(Exclude = false)]
        public static IntPtr WebPDecodeBGRAInto(IntPtr data, long data_size, IntPtr output_buffer, long output_buffer_size, int output_stride)
        {
            switch (IntPtr.Size)
            {
                case 4:
                    return WebPDecodeBGRAInto_x86(data, (UIntPtr)data_size, output_buffer, (UIntPtr)output_buffer_size, output_stride);
                case 8:
                    return WebPDecodeBGRAInto_x64(data, (UIntPtr)data_size, output_buffer, (UIntPtr)output_buffer_size, output_stride);
                default:
                    throw new InvalidOperationException("Invalid platform. Can not find proper function");
            }
        }
        ...
        [Obfuscation(Exclude = false)]
        public static VP8StatusCode WebPDecode(IntPtr data, long data_size, ref WebPDecoderConfig webPDecoderConfig)
        {
            switch (IntPtr.Size)
            {
                case 4:
        ...
        public static int WebPEncodeBGR(IntPtr bgr, int width, int height, int stride, float quality_factor, out IntPtr output)
        {
            ...
            {
                case 4:
                    return (int)WebPEncodeBGR_x86(bgr, width, height, stride, quality_factor, out output);
                case 8:
                    return (int)WebPEncodeBGR_x64(bgr, width, height, stride, quality_factor, out output);
                default:
                    throw new InvalidOperationException("Invalid platform. Can not find proper function");
            }
        }
        ...
        private static extern UIntPtr WebPEncodeBGR_x86([InAttribute()] IntPtr bgr, int width, int height, int stride, float quality_factor, out IntPtr output);
        [Obfuscation(Exclude = false)]
        [DllImport("libwebp_x64.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPEncodeBGR")]
        private static extern UIntPtr WebPEncodeBGR_x64([InAttribute()] IntPtr bgr, int width, int height, int stride, float quality_factor, out IntPtr output);
        ...
        public static int WebPEncodeBGRA(IntPtr bgra, int width, int height, int stride, float quality_factor, out IntPtr output)
        {
            ...
            {
                case 4:
                    return (int)WebPEncodeBGRA_x86(bgra, width, height, stride, quality_factor, out output);
                case 8:
                    return (int)WebPEncodeBGRA_x64(bgra, width, height, stride, quality_factor, out output);
                default:
                    throw new InvalidOperationException("Invalid platform. Can not find proper function");
            }
        }
        ...
        private static extern UIntPtr WebPEncodeBGRA_x86([InAttribute()] IntPtr bgra, int width, int height, int stride, float quality_factor, out IntPtr output);
        [Obfuscation(Exclude = false)]
        [DllImport("libwebp_x64.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPEncodeBGRA")]
        private static extern UIntPtr WebPEncodeBGRA_x64([InAttribute()] IntPtr bgra, int width, int height, int stride, float quality_factor, out IntPtr output);
        ...
        public static int WebPEncodeLosslessBGR(IntPtr bgr, int width, int height, int stride, out IntPtr output)
        {
            ...
            {
                case 4:
                    return (int)WebPEncodeLosslessBGR_x86(bgr, width, height, stride, out output);
                case 8:
                    return (int)WebPEncodeLosslessBGR_x64(bgr, width, height, stride, out output);
                default:
                    throw new InvalidOperationException("Invalid platform. Can not find proper function");
            }
        }
        ...
        public static int WebPEncodeLosslessBGRA(IntPtr bgra, int width, int height, int stride, out IntPtr output)
        {
            ...
            {
                case 4:
                    return (int)WebPEncodeLosslessBGRA_x86(bgra, width, height, stride, out output);
                case 8:
                    return (int)WebPEncodeLosslessBGRA_x64(bgra, width, height, stride, out output);
                default:
                    throw new InvalidOperationException("Invalid platform. Can not find proper function");
            }
        }
    ...
    ///<summary>Color spaces for chroma sampling.</summary>
    [Obfuscation(Exclude = true)]
    enum WebPEncCSP
    {
        ///<summary>4:2:0</summary>
        WEBP_YUV420 = 0,
        /// <summary> alpha channel variant </summary>
        WEBP_YUV420A = 4,
    ...
    {
        /// <summary> final buffer (of size 'max_size', larger than 'size').</summary>
        public IntPtr mem;
        /// <summary> final size</summary>
        public UIntPtr size;
    ...
    #region | WebpException |
    /// <summary>WebPWrapper Exception base class</summary>
    [Obfuscation(Exclude = true)]
    [Serializable()]
    public class WebPException : Exception
    {
        /// <summary>Set Exception message to be "thrown Exception in WebPWrapper." </summary>
        public WebPException() : base( "thrown Exception in WebPWrapper." ) { }
        /// <summary>Set Exception message to message plus " thrown Exception in WebPWrapper." </summary>
        /// <param name="message">
        /// Message string for First addition. "{message} thrown Exception In WebP Wrapper."
        /// </param>
        public WebPException(string message)
            : base( message + " thrown Exception in WebPWrapper." ) { }
        /// <summary>Set Exception message to be "{message} thrown Exception in WebPWrapper.{innerException.TargetSite}" </summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message} thrown Exception in WebPWrapper.{innerException.TargetSite}"
        /// </param>
        /// <param name="innerException">The rized exception. And also for targetSite name.</param>
        public WebPException(string message, Exception innerException) 
            : base( message + " thrown Exception in WebPWrapper." + innerException.TargetSite, innerException ) { }
        ///<remarks>逆シリアル化コンストラクタ。このクラスの逆シリアル化のために必須。アクセス修飾子をpublicにしないこと！</remarks>
        protected WebPException(SerializationInfo info, StreamingContext context) : base(info, context) { }
    }
    /// <summary>WebP library exception class.</summary>
    [Obfuscation(Exclude = true)]
    [Serializable()]
    public class WebPLibException : WebPException
    {
        /// <summary>Set Exception message to be "At Library, thrown Exception in WebPWrapper."</summary>
        public WebPLibException() : base ( "At Library" ) { }
        /// <summary>Set Exception message. To be "{message} at Library, thrown Exception In WebPWrapper."</summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message} at Library, thrown Exception In WebPWrapper."
        /// </param>
        public WebPLibException(string message)
            : base( message + "at Library") { }
        /// <summary>Set Exception message to be "{message}at Library thrown Exception in WebPWrapper.{innerException.TargetSite}" </summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message}at Library thrown Exception in WebPWrapper.{innerException.TargetSite}"
        /// </param>
        /// <param name="innerException">The rized exception. And also for targetSite name.</param>
        public WebPLibException(string message, Exception innerException)
            : base( message + "at Library", innerException) { }
        ///<remarks>逆シリアル化コンストラクタ。このクラスの逆シリアル化のために必須。アクセス修飾子をpublicにしないこと！</remarks>
        protected WebPLibException(SerializationInfo info, StreamingContext context) : base(info, context) { }
    }
    /// <summary>WebP library exception class.</summary>
    [Obfuscation(Exclude = true)]
    [Serializable()]
    public class WebPDllNotFoundException : WebPException
    {
        /// <summary>Set Exception message to be "WebP DLL Not Found thrown Exception in WebPWrapper."</summary>
        public WebPDllNotFoundException() : base("WebP DLL Not Found") { }
        /// <summary>Set Exception message. To be "{message} at Library, thrown Exception In WebPWrapper."</summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message}WebP DLL Not Found thrown Exception In WebPWrapper."
        /// </param>
        public WebPDllNotFoundException(string message)
            : base(message + "WebP DLL Not Found") { }
        /// <summary>Set Exception message to be "{message}, WebP DLL Not Found thrown Exception in WebPWrapper.{innerException.TargetSite}" </summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message}WebP DLL Not Found thrown Exception in WebPWrapper.{innerException.TargetSite}"
        /// </param>
        /// <param name="innerException">The rized exception. And also for targetSite name.</param>
        public WebPDllNotFoundException(string message, Exception innerException)
            : base(message + "WebP DLL Not Found", innerException) { }
        ///<remarks>逆シリアル化コンストラクタ。このクラスの逆シリアル化のために必須。アクセス修飾子をpublicにしないこと！</remarks>
        protected WebPDllNotFoundException(SerializationInfo info, StreamingContext context) : base(info, context) { }
    }
    /// <summary>WebPDecoder Exception base class</summary>
    [Obfuscation(Exclude = true)]
    [Serializable()] //クラスがシリアル化可能であることを示す属性
    public class WebPDecoderException : WebPException
    {
        /// <summary>Set Exception message to be "At Decoder, thrown Exception in WebPWrapper."</summary>
        public WebPDecoderException() : base ("At Decoder") { }
        /// <summary>Set Exception message. To be "{message} at Decoder, thrown Exception In WebPWrapper."</summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message} at Decoder thrown Exception In WebPWrapper."
        /// </param>
        public WebPDecoderException(string message)
            : base( message + "at Decoder") { }
        /// <summary>Set Exception message to be "{message} at Decoder thrown Exception in WebPWrapper.{innerException.TargetSite}" </summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message}at Decoder thrown Exception in WebPWrapper.{innerException.TargetSite}"
        /// </param>
        /// <param name="innerException">The rized exception. And also for targetSite name.</param>
        public WebPDecoderException(string message, Exception innerException)
            : base( message + "at Decoder", innerException) { }
        ///<remarks>逆シリアル化コンストラクタ。このクラスの逆シリアル化のために必須。アクセス修飾子をpublicにしないこと！</remarks>
        protected WebPDecoderException(SerializationInfo info, StreamingContext context) : base(info, context) { }
    }
    /// <summary>WebPEncoder Exception base class</summary>
    [Obfuscation(Exclude = true)]
    [Serializable()] //クラスがシリアル化可能であることを示す属性
    public class WebPEncoderException : WebPException
    {
        /// <summary>Set Exception message to be "At Encoder, thrown Exception in WebPWrapper."</summary>
        public WebPEncoderException() : base ("At Encoder,") { }
        /// <summary>Set Exception message. To be "{message} at Encoder, thrown Exception In WebPWrapper."</summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message}at Encoder, thrown Exception In WebPWrapper."
        /// </param>
        public WebPEncoderException(string message) : base(message + "at Encoder") { }
        /// <summary>Set Exception message to be "{message}at Encoder, thrown Exception in WebPWrapper.{innerException.TargetSite}" </summary>
        /// <param name="message">
        /// Message string for First addition. To be "{message}at Encoder, thrown Exception in WebPWrapper.{innerException.TargetSite}"
        /// </param>
        /// <param name="innerException">The rized exception. And also for targetSite name.</param>
        public WebPEncoderException(string message, Exception innerException) : base(message + "at Encoder", innerException) { }
        //逆シリアル化コンストラクタ。このクラスの逆シリアル化のために必須。
        //アクセス修飾子をpublicにしないこと！
        protected WebPEncoderException(SerializationInfo info, StreamingContext context) : base(info, context) { }
    }
    #endregion
}